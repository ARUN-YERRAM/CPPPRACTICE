...
int Recrse(int wt[], int val[], int n, int W, vector<vector<int>>& t) {
    ...
    if (wt[n-1] <= W) {
        // Include the current item and do not reduce n (because we can reuse the item)
        return t[n][W] = max(val[n-1] + Recrse(wt, val, n, W-wt[n-1], t), 
                             Recrse(wt, val, n-1, W, t)); // Exclude the current item
    } else {
...
int Tabulation(int wt[], int val[], int n, int W) {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= W; j++) {
            if (wt[i-1] <= j) {
                // Include the current item and don't skip it (as we can reuse the item)
