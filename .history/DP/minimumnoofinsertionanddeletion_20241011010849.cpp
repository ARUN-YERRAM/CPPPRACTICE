...
int main() {
    ...
    // Calculate LCS
    int lcsLength = RecrseTab(s1, s2, n, m);

    // Minimum deletions = s1.length() - LCS
    int deletions = n - lcsLength;

    // Minimum insertions = s2.length() - LCS
    int insertions = m - lcsLength;

    cout << "Minimum deletions: " << deletions << endl;
    cout << "Minimum insertions: " << insertions << endl;
    cout << "Total operations (insertions + deletions): " << (deletions + insertions) << endl;
