...
bool RecurseScramb(string s1, string s2) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        // Case 2: Swap
        if (RecurseScramb(s1.substr(0, i), s2.substr(n - i,i)) && 
            RecurseScramb(s1.substr(i,n-i), s2.substr(0, n - i))) {
            return true;
        }
    }
    return false;
...
bool RecurseScrambMemo(string s1, string s2) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        // Case 2: Swap
        if (RecurseScrambMemo(s1.substr(0, i), s2.substr(n - i,i)) && 
            RecurseScrambMemo(s1.substr(i,n-i), s2.substr(0, n - i))) {
            return memo[key] = true;
        }
    }
...
bool RecurseScrambMemo(string s1,string s2,int i,int j){
